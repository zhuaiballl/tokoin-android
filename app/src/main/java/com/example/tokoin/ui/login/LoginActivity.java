...
//                    Tokoin contract = Tokoin.load("0xefc116eda4114f734a7ca902124b38856dab83be",web3j,credentials,contractGasProvider);
                    ...
                    if(opt=="Create Tokoin"){
                        try {
                            res = network.sendCreate(""+holderEditText.getText()+redeemTimeEditText.getText()+itemIdEditText.getText());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }else if(opt=="Modify Tokoin"){
                        try {
                            res = network.sendCreate(""+itemIdEditText.getText()+redeemTimeEditText.getText()+tidSpinner.getSelectedItem().toString());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }else if(opt=="Transfer Tokoin"){
                        try {
                            res = network.sendCreate(""+addressSpinner.getSelectedItem().toString()+holderEditText.getText()+tidSpinner.getSelectedItem().toString());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }else if(opt=="Delete Tokoin"){
                        try {
                            res = network.sendCreate(""+tidSpinner.getSelectedItem().toString());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
